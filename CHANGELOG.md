...
- **Dependency Update**: Updated `@tijs/hono-oauth-sessions` to 2.2.0 (critical
  bug fix)
  - Fixes iron-session cookie refresh: cookies are now properly refreshed on
    each request
  - Sessions now maintain TTL based on last activity, not original login time
  - This resolves sessions expiring after 1-2 hours instead of the configured 14
    days

### Added

- **Re-export `OAuthSessionFromRequestResult`**: New type re-exported from
  `hono-oauth-sessions`
  - Enables applications to use the new `getOAuthSessionFromRequestWithCookie()`
    method
  - Includes typed error information for better diagnostics
...
- Updated `@tijs/hono-oauth-sessions` to 2.1.3 (fixes session persistence with
  TTL)
